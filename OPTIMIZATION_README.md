...
| `4f38432` | Qwen3.5 DTree | Child-margin tree scoring | Rewarded prompt-specific top-1 continuation confidence so the heap spends more budget on depths where the next draft token is decisive | Quick q4 screen with `DTREE_TREE_SCORE_MODE=child_margin` dropped acceptance from `7.06` to `6.29` and DTree from `50.43` to `46.16` e2e TPS | 1.02x short, much worse absolute TPS | Drop |
